...
const INITIAL_MATCHES = [
  { id: 1, date: '11.06', time: '22:00', home: 'Мексико', away: 'Южна Африка', oddsH: 1.35, oddsD: 4.80, oddsA: 9.00, status: 'upcoming', resultHome: null, resultAway: null }
];
...
export default function App() {
  ...
  // Инициализация на Firebase Auth
  useEffect(() => {
  ...
  const handleLogin = async (e) => {
    ...
    if (loginName.trim() === 'Admin' && loginPass.trim() === ADMIN_PIN) {
        setIsAdmin(true);
    ...
    const newUsers = users.map(u => {
      if (u.name === currentUser) {
        ...
        if (h === '' && a === '') {
            ...
            newPreds[matchId] = { h: h === '' ? '' : parseInt(h), a: a === '' ? '' : parseInt(a) };
        }
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 pb-20">
      ...
      <main className="p-4 max-w-2xl mx-auto">
        ...
        {/* Бутони Предстоящи / История за играчите */}
        {activeTab === 'matches' && (
        ...
        {activeTab === 'matches' && matches
          ...
          .map(m => {
            ...
            const hVal = typeof myPred.h === 'object' ? '' : (myPred.h ?? '');
            const aVal = typeof myPred.a === 'object' ? '' : (myPred.a ?? '');
            ...
            return (
              <div key={m.id} className="bg-slate-800 p-4 rounded-xl border border-slate-700 mb-4 shadow-sm relative overflow-hidden">
                ...
                {/* НОВИЯТ КРАСИВ ИЗГЛЕД ЗА ПРОГНОЗИТЕ (С GRID) */}
                <div className="mt-4 pt-3 border-t border-slate-700/50">
                  <div className="text-xs text-slate-500 mb-2 uppercase tracking-wide flex justify-between items-center">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {users
                      ...
                      .map(u => {
                        ...
                        const safeH = typeof p.h === 'object' ? '?' : p.h;
                        const safeA = typeof p.a === 'object' ? '?' : p.a;
                        
                        const hasFullPred = (safeH !== '' && safeA !== '' && safeH !== '?');
                        const pts = calculatePoints(m, p);
                             ...
                             {isVisible ? (
                                  hasFullPred ? (
                                      <div className="flex flex-col items-end">
                                          <span className={isFinished && pts > 0 ? 'text-emerald-400 font-bold' : 'text-slate-400 font-mono'}>
                                              {safeH} - {safeA}
                                          </span>
                                          {isFinished && pts > 0 && <span className="text-[9px] text-emerald-500/80">+{pts}т.</span>}
                                      </div>
                                  ) : (
                                      <span className="text-slate-500 text-[10px] uppercase">Непълна</span>
                                  )
                             ) : (
                                  <span className="text-slate-500 text-[10px] uppercase tracking-wider flex items-center gap-1"><Lock size={10}/></span>
                             )}
                           </div>
                        );
                    })}
